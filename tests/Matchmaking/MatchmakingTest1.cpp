...
void deleteGame(string gameId) {
    DBConnection::instance().ExecParams("DELETE FROM games WHERE id=$1", { gameId });
}
...
TEST(Matchmaking, DeleteGame)
{
    ...
    deleteGame(id_res.field_by_name(0, "id"));
...
TEST(Matchmaking, CreateGame)
{
    ...
    deleteGame(newGame.field_by_name(0, "id"));
}
...
TEST(Matchmaking, HostLeaveGame)
{
    ...
    if (res.row_count())
        deleteGame(res.field_by_name(0, "id"));
    else
