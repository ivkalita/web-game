...
Response Matchmaking::createGame(Game game, int playerId) {
    ...
    string id;
    int host = playerId;
    string gameId = isInGame(playerId);
    if (gameId.size())
        throw MatchmakingException(Response::NOT_CREATED);
...
Response Matchmaking::onLeaveGame(Poco::JSON::Array::Ptr params) {
    try {
        ...
        string gameId;
        string accessToken = params->getElement<string>(0);
        int uId = User(accessToken).getId();
        gameId = isInGame(uId);
        return OnLeaveGame(to_string(uId), gameId);
    }
    catch (MatchmakingException &e) {
        cout << "Error: " << e.what() << endl;
        Response r;
...
string Matchmaking::isInGame(int player_id) {
    auto res = DBConnection::instance().ExecParams("SELECT game_id FROM CONNECTIONS WHERE player_id=$1", { to_string(player_id) });
    if (res.row_count())
        return string(res.field_by_name(0, "game_id"));
    else
        return string();
}

Response Matchmaking::OnLeaveGame(string userId, string gameId) {
    std::ostringstream osstream;
    string ownerid;
    string exceptId;
    string text;

    auto playerList = getPlayersList(stoi(gameId));
    Response r = Matchmaking::leaveGame(stoi(userId), NULL, &ownerid);
    if (ownerid == userId) {
        exceptId = ownerid;
        Response(NULL, Response::HOST_LEAVE_LOBBY, Actions::getActionText(Actions::LEAVE_GAME)).toJson().stringify(osstream);
    }else{
        exceptId = userId;
        getLobbyInfo(stoi(ownerid)).toJson().stringify(osstream);
    }
    text = osstream.str();
    for (auto player : playerList ) {
        if (player != exceptId)
            ConnectionsPoll::instance().sendMessage(text, stoi(player));
    }
    return r;
}
...
void Matchmaking::onCloseConnection(int id) {
    ...
    gameId = isInGame(id);
    if (gameId.size());
        OnLeaveGame(to_string(id), gameId);
}
